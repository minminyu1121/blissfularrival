...
export default function LoginForm() {
  ...
  return (
    <div className="flex w-full flex-col gap-5">
      {error && (
        <p className="rounded-2xl bg-red-50 px-4 py-2.5 text-sm text-red-500 ring-1 ring-red-100">
          {error}
        </p>
      )}
...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true">
      ...
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="white" fillOpacity="0.75" />
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="white" fillOpacity="0.75" />
...
function translateFirebaseError(message: string): string {
  if (message.includes("popup-closed-by-user")) return "已取消登入";
  if (message.includes("cancelled-popup-request")) return "登入視窗被中斷，請再試一次";
  if (message.includes("account-exists-with-different-credential"))
    return "此電子郵件已用其他方式註冊";
  if (message.includes("network-request-failed")) return "網路連線失敗，請檢查網路後再試";
  if (message.includes("operation-not-allowed"))
    return "Google 登入尚未啟用，請至 Firebase Console 開啟";
